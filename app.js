...
function downloadInvoice(order) {
  ...
  const delivery = order.shippingCharge == null ? "Shiprocket quote pending" : order.shippingCharge === 0 ? "FREE" : formatPrice(order.shippingCharge);
  const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Invoice ${escapeHTML(order.orderReference)}</title><style>*{box-sizing:border-box}body{margin:0;background:#f6efe4;color:#2c211d;font-family:Arial,sans-serif}.invoice{max-width:850px;margin:24px auto;background:#fff;border-top:8px solid #9e2f1c;box-shadow:0 10px 30px #5b2b1b22}.header{display:flex;justify-content:space-between;gap:20px;padding:24px 30px;background:linear-gradient(135deg,#fff8ec,#fde5b7)}.brand{display:flex;align-items:center;gap:16px}.brand img{width:82px;height:82px;object-fit:contain;border-radius:50%;background:#fff}.brand h1{margin:0;color:#7a2114;font-family:Georgia,serif;font-size:25px}.brand p,.meta p{margin:5px 0;font-size:13px}.meta{text-align:right}.title{font-size:26px;color:#9e2f1c;font-weight:800}.content{padding:26px 30px}.details{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:24px}.box{border:1px solid #ead7be;border-radius:12px;padding:16px;background:#fffdf9}.box h3{margin:0 0 10px;color:#7a2114;font-size:14px;text-transform:uppercase;letter-spacing:.8px}.box p{margin:5px 0;line-height:1.5}table{width:100%;border-collapse:collapse;margin:18px 0}th{background:#9e2f1c;color:#fff;text-align:left;padding:11px}td{padding:11px;border-bottom:1px solid #ead7be}th:nth-child(n+3),td:nth-child(n+3){text-align:right}.summary{margin-left:auto;width:min(100%,340px);border:1px solid #ead7be;border-radius:12px;padding:14px}.summary p{display:flex;justify-content:space-between;margin:9px}.summary .grand{border-top:2px solid #9e2f1c;padding-top:12px;font-size:19px;color:#7a2114}.note{margin-top:22px;background:#fff2d8;border-left:5px solid #e5a52f;padding:14px;line-height:1.5}.footer{text-align:center;padding:18px;background:#2f5c35;color:#fff;line-height:1.6}.print{display:block;margin:20px auto;padding:11px 24px;border:0;border-radius:8px;background:#9e2f1c;color:#fff;font-weight:bold;cursor:pointer}@media(max-width:600px){.header,.details{display:block}.meta{text-align:left;margin-top:18px}.content{padding:20px 14px}.invoice{margin:0}.box{margin-bottom:12px}th,td{padding:8px 5px;font-size:12px}}@media print{body{background:#fff}.invoice{margin:0;box-shadow:none}.print{display:none}}</style></head><body><main class="invoice"><header class="header"><div class="brand"><img src="https://maligesivappa.github.io/sree-veerabhadra-pickles/logo.jpeg" alt="Sree Veerabhadra logo"><div><h1>Sree Veerabhadra<br>Homemade Foods</h1><p>Traditional taste, prepared with care</p></div></div><div class="meta"><div class="title">ORDER INVOICE</div><p><strong>Invoice/Order:</strong> ${escapeHTML(order.orderReference)}</p><p><strong>Date:</strong> ${new Date().toLocaleDateString("en-IN")}</p><p><strong>Status:</strong> ${escapeHTML(order.status || "New")}</p></div></header><section class="content"><div class="details"><div class="box"><h3>From</h3><p><strong>Sree Veerabhadra Homemade Foods</strong><br>Opposite Bus Stand, near Rajappa Hospital<br>Velgode – 518533, Nandyal, Andhra Pradesh<br>Phone/WhatsApp: +91 94902 10173</p></div><div class="box"><h3>Bill & Deliver To</h3><p><strong>${escapeHTML(customer.name || "Customer")}</strong><br>${escapeHTML(customer.phone || "")}<br>${escapeHTML(customer.address || "")}<br>${escapeHTML(customer.city || "")}, ${escapeHTML(customer.state || "")} - ${escapeHTML(customer.pincode || "")}</p></div></div><table><thead><tr><th>#</th><th>Product</th><th>Qty</th><th>Price</th><th>Amount</th></tr></thead><tbody>${itemRows}</tbody></table><div class="summary"><p><span>Items subtotal</span><strong>${formatPrice(order.subtotal)}</strong></p><p><span>Coupon discount</span><strong>-${formatPrice(order.discount || 0)}</strong></p><p><span>Shipping</span><strong>${delivery}</strong></p><p class="grand"><span>Total</span><strong>${formatPrice(order.total)}</strong></p><p><span>Payment</span><strong>${escapeHTML(order.paymentMethod || "COD")}</strong></p></div><div class="note"><strong>Thank you for your order!</strong><br>Our food is freshly prepared after receiving your order. Free delivery is available when the product subtotal is ₹2,499 or more.</div><button class="print" onclick="window.print()">Print / Save as PDF</button></section><footer class="footer">Sree Veerabhadra Homemade Foods<br>Delivery Across India • WhatsApp: +91 94902 10173</footer></main></body></html>`;
